...
3. Ensure pgbackrest is configured exactly the same as the primary
   copy stanza
4. Stop postgres
...
7. `cp /var/lib/postgresql/15/main/postgresql.auto.conf /etc/postgresql/15/main/`
   - File seems to be generated by pgbackrest, but it seems to be created in the wrong location. [@albertchae](https://github.com/albertchae) will investigate.
