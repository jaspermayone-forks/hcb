...
RSpec.describe Ledger::AssertCtsSyncedWithHcbCodeJob do
  ...
  def canonical_transaction(hcb_code:, ledger_item:)
    create(:canonical_transaction).tap do |ct|
      ct.update_columns(hcb_code:, ledger_item_id: ledger_item&.id)
    end
  end

  it "reports nothing when a transaction and its HCB code agree" do
    canonical_transaction(hcb_code: hcb_code.hcb_code, ledger_item: item)
  ...
  it "reports nothing when neither has a ledger item" do
    ...
    canonical_transaction(hcb_code: hcb_code.hcb_code, ledger_item: nil)

    expect(anomalies).to be_empty
  end

  it "reports a transaction on a different ledger item than its HCB code" do
    ct = canonical_transaction(hcb_code: hcb_code.hcb_code, ledger_item: create(:ledger_item))

    expect(anomalies).to contain_exactly(a_string_including("CanonicalTransaction #{ct.id}"))
  end

  it "reports a transaction with no ledger item whose HCB code has one" do
    ct = canonical_transaction(hcb_code: hcb_code.hcb_code, ledger_item: nil)

    expect(anomalies).to contain_exactly(a_string_including("CanonicalTransaction #{ct.id}"))
  end

  it "reports a transaction on a ledger item whose HCB code doesn't exist" do
    ct = canonical_transaction(hcb_code: "HCB-000-missing", ledger_item: item)
